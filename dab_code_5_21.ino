...
void readGPS() {
  ...
  if (millis() - timer > 2000) {
    ...
    Serial.print("\nTime: ");
    Serial.print(GPS.hour, HEX); //time in shorts
    //gives DEC -> HEX
    Serial.print(':');
...
void readBME() {
  ...
  BMEData.close();
   ...
   //union between byte array and XYZ data float
/*
    union {byte b[4]; float xyz;}; //float is double
    xyz = DataX;
    for (int i = 0; i < 4; i++){
    xbee.write(b[i]);
    }
    ...
    xyz = DataZ;
    union {byte b[4]; float dataZ;}
    for (int i = 0; i < 4; i++){
    Serial.write(b[i]);
    }
    
    union {byte b[4]; float temp;}
    }
