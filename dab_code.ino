...
void setup()
{
  ...
  myTimer1.begin(readBNO, 1000000);  // readBNO to run every 1 seconds
  ...
  myTimer3.begin(readBME, 1000000);  // readBME to run every 1 seconds
}
...
void readGPS() {
  GPSData = SD.open("GPS.txt", FILE_WRITE);
  char c = GPS.read();
  if (GPSECHO) // REMOVE THIS PARAMETER AND TEST
    if (c) GPSSerial.print(c);
  ...
  if (millis() - timer > 2000) {
    ...
    if (GPS.fix) {
      ...
      GPSData.print(GPS.latitude, 4); GPSData.print(GPS.lat);
      GPSData.print(", ");
      GPSData.print(GPS.longitude, 4); GPSData.println(GPS.lon);
